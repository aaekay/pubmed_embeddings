...
- Embeddings (TEI): `EMBEDDING_SOURCE=tei` (alias `tie`) uses `TEI_BASE_URL` and batched `POST /embed` with `TEI_BATCH_SIZE` / `--tei-batch-size`; `embedding_source` stored in `state.sqlite` meta; README + `.env.example` updated; smoke: `pubmed-embed --help`, TEI parse helpers, TEI progress bar hits dead port with temp SQLite.
